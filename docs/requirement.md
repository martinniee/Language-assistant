...
- 新增/编辑单词面板中的备注、定义、例句输入框支持紧凑型 Markdown 格式工具栏；输入框获得焦点后显示工具栏，失焦后隐藏，不为每个输入框常驻占用空间。
- Markdown 格式工具栏支持粗体、斜体、红/蓝/绿颜色和 Markdown 链接；粗体写入 `**文本**`，斜体写入 `*文本*`，颜色写入 `<font color="#ff0000|#0000ff|#008000">文本</font>`，链接写入 `[显示文本](链接地址)`。
- Markdown 格式工具栏应支持格式切换：已格式化文本再次点击同类格式可取消；颜色同色再次点击取消颜色，异色点击替换颜色；颜色取消后只保留普通文本。
- Markdown 链接编辑应支持外部 URL 与 Obsidian 内部 Markdown 路径；未选中文本时允许插入 `[]()`；选中已有链接或光标位于已有链接内时可编辑显示文本、链接地址，并可移除链接。
- 新增/编辑单词面板中定义和例句仍支持拖拽排序，但从输入框、格式工具栏、链接弹层等交互控件发起的鼠标拖动不得触发排序拖拽，以保证文本可以正常拖选。
...
- 支持可选全文高亮。
- 支持按列表或网格模式展示单词。
- 单词展示卡片应直接展示查询次数徽章，徽章仅作信息展示，不提供点击交互，不改变查询次数更新逻辑。
- 支持分页浏览。
...
- 删除词性、定义、例句时需要确认。
- 新增/编辑面板支持调整定义和例句顺序：定义只能在同一词性内排序；例句可在同一词性下跨定义移动；暂不支持词性块排序。拖拽时应根据指针所在目标项的上半区/下半区显示前/后插入高亮横线，并提供可见的上/下移图标按钮作为移动端和键盘操作兜底。排序只改变数组顺序，不新增数据字段。
...
- 新增/编辑单词面板的 Markdown 格式工具栏只负责写入兼容 Markdown 的原始文本，不新增数据字段，不改变 `Word` 数据结构、Markdown 序列化格式或历史数据读取策略。
- 颜色快捷格式统一写入 `<font color="#ff0000">文本</font>`、`<font color="#0000ff">文本</font>`、`<font color="#008000">文本</font>`；再次选择同色时应移除外层颜色标签，仅保留内部文本；选择异色时应替换外层颜色值。
- Markdown 链接快捷格式统一写入 `[显示文本](链接地址)`；链接地址可为外部 URL，也可为 Obsidian 内部 Markdown 路径或锚点路径，存储层不做 URL 协议限制。
...
- 新增/编辑面板支持定义和例句排序；定义块可在同一词性内拖放或通过上/下移按钮调整，例句可在同一词性下拖到其他定义或通过上/下移按钮跨相邻定义移动；拖拽过程中根据目标项上半区/下半区展示前/后插入高亮横线，落点与实际数组插入位置一致；上/下移按钮使用可见图标并覆盖全局按钮尺寸样式；该改动只改变 `content[].definitions[]` 与 `definition.examples[]` 的顺序，不改变 Markdown 存储结构。

### 2026-06-11

- 新增 `MarkdownFormatField` 通用输入组件，用于新增/编辑单词面板中的备注、定义、例句输入框；输入框获得焦点后显示紧凑 Markdown 格式工具栏，工具栏失焦后隐藏，避免固定工具栏长期占用编辑空间。
- Markdown 格式工具栏支持粗体、斜体、红/蓝/绿颜色和 Markdown 链接快捷写入；粗体与斜体沿用 Markdown 标记，颜色使用 `<font color="#ff0000|#0000ff|#008000">文本</font>`，链接使用 `[显示文本](链接地址)`。
- 颜色快捷操作支持切换语义：普通文本点击颜色时添加颜色标签；选中已有颜色标签整体或标签内部文本时，同色点击移除颜色、异色点击替换颜色；取消颜色后仅保留普通文本。
- 链接快捷操作支持未选中文本时插入 `[]()`；选中已有链接或光标位于已有链接内时可编辑显示文本与链接地址，并可移除链接；链接地址兼容外部 URL 与 Obsidian 内部 Markdown 路径。
- 修复定义/例句拖拽排序与文本鼠标拖选冲突：从输入框、格式工具栏、链接弹层等交互控件发起的拖动不触发排序拖拽，保留正常文本选择能力，同时不移除原有排序功能。
- 工具栏视觉改为小巧紧凑样式：缩小按钮、色点、间距、阴影与链接弹层尺寸；该调整只影响编辑体验与样式，不改变 `Word` 数据结构、Markdown 存储格式、Obsidian view id、命令或设置字段。
